...
# Linux (CI: .github/workflows/build-deb.yml). The package installs the web
# UI app under /opt/dj-cratebuilder; its postinst builds a --system-site-
# packages venv there and installs requirements.txt into it — the same
# pattern as install-linux.sh. Requires Pillow (python3-pil) for the
# icon.ico → PNG conversion.
# ============================================================================
set -e
...
# ── Application payload ───────────────────────────────────────────────────
# DJ-CrateBuilder_v2.0.py ships as source text only — cratebuilder/service.py
# parses it for version/About data (MAIN_SCRIPT); the app itself is
# web_window.py + web/.
cp "$ROOT/$SCRIPT_NAME" "$PKG$APP/"
cp "$ROOT/web_window.py" "$PKG$APP/"
cp "$ROOT/web_server.py" "$PKG$APP/"
cp -r "$ROOT/web" "$PKG$APP/web"
...
#!/bin/sh
exec "$APP/venv/bin/python" "$APP/web_window.py" "\$@"
EOF
...
Priority: optional
Architecture: all
Depends: python3 (>= 3.10), python3-venv, python3-gi, python3-gi-cairo, gir1.2-gtk-3.0, gir1.2-webkit2-4.1 | gir1.2-webkit2-4.0, ffmpeg
...
Description: Batch download audio from YouTube and SoundCloud as MP3
 Desktop web-UI app for DJs: batch-downloads audio from YouTube and
 SoundCloud channels, playlists, and individual videos/tracks as MP3 files,
 organised into ~/Music/DJ-CrateBuilder/<Platform>/<Genre>/<Channel>/.
 .
 Python dependencies (yt-dlp, pywebview, fastapi, ...) are installed into a
 private virtual environment (--system-site-packages, for PyGObject) under
 /opt/dj-cratebuilder at install time.
EOF
