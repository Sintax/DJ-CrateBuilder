...
TOOLTIPS = {
    ...
    'settings.browser_handler': 'Lets the browser extension send YouTube and SoundCloud channels and tracks straight to this app by registering the djcrate:// link type with Windows — per-user, no admin rights. Untick and re-tick after moving or reinstalling the app to refresh the stored path. On Linux the .desktop entry does this instead.',
    'settings.browser_integration': "The djcrate:// link type the DJ-CrateBuilder browser extension uses to hand this app a channel or a track. It is one-way: the extension can't tell whether the app received a send, so if one seems lost, check the handler toggle here first.",
    'settings.browser_receive_mode': "What happens when the extension sends a link. Bring window forward opens the matching dialog at once, prefilled. Collect quietly queues it in the Browser Inbox on the Watch List and pings the tray instead, so a run of sends doesn't keep pulling the window up.",
    'settings.cookie_file': 'Netscape/Mozilla cookie.txt format',
    'settings.cookies': 'Authenticate with a YouTube and/or SoundCloud account for faster downloads and fewer "login required" errors. Use a dedicated/throwaway account — never your personal one. SoundCloud Go+ tracks need the subscribed account, which carries the same risk. Chrome 127+ blocks cookie extraction; use Firefox or a cookie file.',
    ...
    'wl.browser_inbox': 'Links the browser extension sent while receive mode was Collect quietly. Process opens the same prefilled dialog a send would have opened straight away; Remove discards it.',
    ...
    'wl.conflict_apply_all': 'Answer every remaining clash in this import the same way, without showing you the entries. Use with extreme caution.',
    'wl.conflict_new': "Keep your entry and add the file's channel as a second entry under a name you type. Not offered when the link itself is already tracked.",
    'wl.conflict_new_name': "The name for the new entry. It is also the channel's folder name, so it must differ from every other entry.",
    'wl.conflict_overwrite': 'Point your existing entry at the link from the file. Your name and genre for it are kept, so nothing moves on disk.',
    'wl.conflict_skip': 'Leave your entry exactly as it is and move on to the next channel.',
    'wl.conflict_stop': 'Stop the import here. Channels already added or re-pointed stay; the rest are left alone.',
    'wl.download_all_new': 'Download all pending new tracks across every channel.',
    'wl.export_confirm': 'Save the ticked channels to a file. A Save dialog asks where to put it.',
    'wl.export_list': "Choose which channels to save into a file another DJ-CrateBuilder user can import: each channel's link, name, platform and genre. No counts, dates or folder paths from this machine.",
    'wl.import_confirm': 'Add the ticked channels to your Watch List, one at a time. Any that clash with an entry you already have will ask you what to do.',
    'wl.import_list': "Open another user's exported list and choose which channels to add. A channel that is already in your Watch List (same link or same name) asks you what to do — your entries are never changed unless you say so.",
    ...
    'wl.share_all': 'Tick every channel in the list.',
    'wl.share_cancel': 'Close without exporting or importing anything.',
    'wl.share_none': 'Untick every channel in the list.',
    'wl.share_row_check': 'Tick to include this channel.',
    'wl.share_row_unresolved': 'This channel has no real link yet, so there is nothing to share. Use Check Links to resolve it first.',
}
